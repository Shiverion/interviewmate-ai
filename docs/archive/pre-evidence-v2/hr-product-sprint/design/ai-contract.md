...
[Design package](README.md) · [Executable specification](../../../../hr-product-sprint/design/review-brief-contract.cjs) · [Prompt v1](prompts/review-brief-v1.md)
...
[Request fixtures](../../../../hr-product-sprint/design/examples/practice-a.request.json) and `requestSchema` in the executable spec define the exact shape:
...
1. Read the [exact instruction file](prompts/review-brief-v1.md); append two newlines, `TRUSTED_ROLE_PROFILE_JSON:`, a newline and serialized [role profile](../../../../hr-product-sprint/design/role-profile.json). This is the system message.
...
## Phase 3 implementation clarifications

The [runtime runbook](../implementation/review-brief-runbook.md) records the implemented locations and setup. Changing the reviewer identity, like changing the reviewer note, clears every criterion check and the global reviewed attestation. Cancellation returns GENERATION_CANCELLED (HTTP 499) when a response can still be delivered. Unknown-speaker context produces a visible warning. Synthetic provider-attempt records are stored locally; reviewer edits remain in browser memory/export. Pre-provider input/configuration rejection creates no provider-attempt record. The first genuine provider request failed with AI_UNAVAILABLE; successful live generation remains pending.

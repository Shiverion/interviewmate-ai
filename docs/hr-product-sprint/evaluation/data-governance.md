...
The recruiter CV pipeline is a separate, recruiter-triggered product flow. It stores bounded parsed text, extracted identity fields and the deterministic ATS snapshot in the creator-owned `pipeline_candidates` collection so screened and not-invited rows can be reviewed. Creating an invitation copies the relevant role/settings/ATS snapshot into a scheduled session; the original PDF is retained only for an invited session. This storage supports the recruiting workflow and is not an automatic training or continuous-learning feed. Deploy the owner-scoped Firestore rule before using persisted rows.

...
The session-alert follow-up adds five component checks covering silent defaults, sound opt-in/mute/failure, dismissal without clearing evidence, no replay from restored history, and Indonesian threshold messages. All five current guardrail browser scenarios pass, including keyboard dismissal and optional-sound controls. These exercise UI/audio calls; audible playback and interference during a real voice interview need human validation.

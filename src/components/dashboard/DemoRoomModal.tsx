...
async function extractTextFromPdfFile(file: File): Promise<string> {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        payload.error || `Resume parse failed: ${response.status}`
      );
    }
    ...
    console.warn(
      "[Demo Room] Failed to parse CV text. Continuing without resume text context.",
      err
    );
    return "";
  }
...
export default function DemoRoomModal({ isOpen, onClose }: DemoRoomModalProps) {
  ...
  const handleStartDemo = async (e: React.FormEvent) => {
    ...
    if (!getOpenAIKey()) {
      setError(
        "Personal voice sessions need an OpenAI key in Models & access. You can also use the free voice demo from Dashboard."
      );
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[var(--background)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-[var(--border)] sticky top-0 bg-[var(--background)] z-10 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="p-2 text-[var(--muted)] hover:text-[var(--foreground)] rounded-full hover:bg-[var(--surface-elevated)] transition-colors"
          >
            <svg
              className="w-5 h-5"
              fill="none"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-2 text-xs text-[var(--muted)]">
                Voice uses microphone conversation. Text uses chat-only
                interaction.
              </p>
            </div>
          </div>

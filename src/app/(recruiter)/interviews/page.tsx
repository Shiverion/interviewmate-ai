...
export default function InterviewsPage() {
  ...
  const handleRevoke = async (sessionId: string) => {
    setConfirmConfig({
      ...
      message:
        "Are you sure you want to revoke this interview link? The candidate will no longer be able to access it.",
      isDestructive: true,
      onConfirm: async () => {
        try {
  ...
  const handleEvaluate = async (sessionId: string) => {
    try {
      ...
      showToast(
        "Error",
        "An error occurred during manual evaluation.",
        "error"
      );
    }
  };
  ...
  const filteredSessions = sessions.filter((session) => {
    const matchesName = session.candidate_name
      ?.toLowerCase()
      .includes(searchQuery.toLowerCase());
    const matchesId = session.candidate_id
      ?.toLowerCase()
      .includes(searchQuery.toLowerCase());
    ...
    const matchesStatus =
      statusFilter === "all" || mapsEvaluated === statusFilter;
                            ...
                            <button
                              onClick={() => {
                                setConfirmConfig({
                                  ...
                                  message:
                                    "This will trigger the AI to evaluate the current transcript immediately. Are you sure?",
                                  onConfirm: () => {

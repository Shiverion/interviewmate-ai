...
export async function getSessionToken(
  ...
  for (const key of candidateKeys) {
    for (const model of candidateModels) {
            ...
            tools: [
              {
                ...
                name: "end_interview",
                description:
                  "Terminates the interview session. Call this tool IMMEDIATELY after you have delivered your professional closing speech and thanked the candidate. DO NOT wait for candidate input before calling this.",
                parameters: {
                  type: "object",
                  properties: {},
                  required: [],
      ...
      lastError = errorData;
      console.error(
        `[Realtime Session] model=${model} status=${response.status} body=${errorData}`
      );
  ...
  throw new Error(
    `Failed to generate session token: ${lastStatus}. ${lastError}`
  );
}

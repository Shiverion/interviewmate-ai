...
- **What was wrong:** the harness compared response bodies with `toStrictEqual` on the result of `response.json()`. Those objects carried a prototype identity that `toStrictEqual` treated as different from the test-file literals, failing with `Received: serializes to the same string`. The likely mechanism (the body parsed in a different JavaScript realm from the test) was inferred from the symptom, not inspected directly; the evidence for the correction is the re-run below. Every one of the 701 `preserved` failures was this; the `observable` check (`:466`) — the one that actually encodes policy — had already passed in each.

...
| Time for one bounded policy change (handoff exercise; the task was finalised in `handoff.md` §4.1 as a `demo` no-grant policy change, not the hosted-admin change first named here) | 7 m 08 s at `50fa2dc` | 16 m 27 s at `2b1ae0f` — longer; behavioural spill-over is now machine-checked, but the author's self-reported "high" confidence at completion was partially falsified post-review by a +42 test-count anomaly (duplicated hand-mirrored rows); see `handoff.md` §4.4 | measured; self-performed by the author, n = 1, AI-assisted, descriptive only |

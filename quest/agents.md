...
|---|---|---|---|
| **Human** | Iqbal | Choose the problem; set the rules (three rounds, unanimity, model tiering); approve the contract table before any route edit (`e25d63e`); accept or reject the branch. **Still pending at v3:** reading the three route diffs and the resolver, the timed handoff exercise, the sign-off in `review-example.md`, the Loom (`effort-log.md`, "not yet done") | Delegate accountability — reviewers advise, the human decides |

...
**Author:** Muhammad Iqbal Hilmy Izzulhaq · **Draft:** v3.1, 2026-09-20 — *initial directive, issued before implementation. Appendix B is an empty template until results exist.*
...
2. Baseline suite: `git checkout 50fa2dc && npx jest --ci` → expect 25 suites / 191 tests (time is machine-dependent; 21.05 s was one run)
3. Baseline violations: `git checkout <H> && npx jest src/app/api/__tests__/evaluation-routes.test.ts --json | jq .numFailedTests`
